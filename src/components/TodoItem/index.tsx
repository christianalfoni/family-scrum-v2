...
const AddCheckListItem: React.FC<{ onAdd: (title: string) => void }> = ({
  onAdd,
}) => {
...
export const TodoItem = React.memo(
  ({
    ...
    const intl = useIntl();
    const checkList = useCheckList(todo.id);

    React.useEffect(() => {
      if (archiving) {
        const id = setTimeout(() => todos.archiveTodo.mutate(todo.id), 1500);
              ...
              <ul className="mt-2">
                {checkList.map((item) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        checkListItems.setCompleted.mutate(
                          item.id,
                          !item.completed,
                        )
                      }
                    ...
                    <span
                      ...
                      onClick={() => checkListItems.removeItem.mutate(item.id)}
                    >
                ...
                <li>
                  {addingCheckListItem ? (
                    <AddCheckListItem
                      onAdd={(title) =>
                        checkListItems.addItem.mutate(todo.id, title)
                      }
                    />
                  ) : (
                    <div
