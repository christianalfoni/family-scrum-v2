...
import { useRef, useSyncExternalStore, FunctionComponent } from "react";
...
export type Signal<T> = {
  get(): T extends Promise<infer V> ? ObservablePromise<V> : T;
  set(value: T): void;
  update(update: (draft: T) => T | void): void;
};
...
export function createSignal<T>(initialValue: T) {
  ...
  return {
    ...
    update(updater) {
      this.set(produce(this.get(), updater));
    },
    set(newValue) {
      if (value === newValue) {
        if (
...
export function createDerived<T>(cb: () => T) {
  let value: T;
  let disposer: () => void;
  let isDirty = true;
  const signal = new SignalTracker(() => value);
