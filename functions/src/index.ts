...
export const barcode = functions.region('europe-west1').https.onRequest((request, response) => {
  ...
  admin.firestore().collection(BARCODE_TOKENS_COLLECTION).doc(token)
    .get()
    .then((doc) => {
      const data = doc.data()
    ...
    .then((familyId) => {
      return admin.firestore().collection(FAMILY_DATA_COLLECTION).doc(familyId).collection(FAMILY_BARCODES_COLLECTION).doc(barcode).get()
        .then((barcodeDoc) => {
          const data = barcodeDoc.data()

          // Scanned and linked to grocery
          if (data && data.groceryId) {
            const groceryRef = admin.firestore().collection(FAMILY_DATA_COLLECTION).doc(familyId).collection(FAMILY_GROCERIES_COLLECTION).doc(data.groceryId)

            return admin.firestore().runTransaction((transaction) => transaction.get(groceryRef).then((groceryDoc) => {
